...
describe('AdminSupportGuard', () => {
  ...
  it('allows an active admin', async () => {
    ...
    await expect(new AdminSupportGuard(prisma).canActivate(context('admin-1'))).resolves.toBe(true);
  });

  it('rejects non-admin users', async () => {
    const prisma = { user: { findUnique: jest.fn().mockResolvedValue({ role: 'USER', isActive: true }) } } as any;
    await expect(new AdminSupportGuard(prisma).canActivate(context('user-1'))).rejects.toThrow('Admin access required.');
  });

  it('rejects inactive admins', async () => {
    const prisma = { user: { findUnique: jest.fn().mockResolvedValue({ role: 'ADMIN', isActive: false }) } } as any;
    await expect(new AdminSupportGuard(prisma).canActivate(context('admin-1'))).rejects.toThrow('Admin access required.');
  });
});
